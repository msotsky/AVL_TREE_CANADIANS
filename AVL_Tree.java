...
public class AVL_Tree {
	...
	/*
	public int countNodes(AVL_Node cur){
		if(cur.getLeftChild() == null && cur.getRightChild() == null)
			return 1;
		else if(cur.getLeftChild() != null && cur.getRightChild() == null)
			return countNodes(cur.getLeftChild()) +1;
		else if (cur.getLeftChild() == null && cur.getRightChild() != null)
			return countNodes(cur.getRightChild()) + 1;
		else
			return countNodes(cur.getLeftChild()) + countNodes(cur.getRightChild()) + 1;
	}
	*/
	...
	public boolean insert(AVL_Node newNode) {
		...
		if(this.root == null){
			...
			while(true){
				...
				else //if (nNode == r) -value already contained
					return false; 
	...
	//^
	//|
	private void changeBF(AVL_Node nNode){

		if(nNode.getBalanceFactor() < -1 || nNode.getBalanceFactor() > 1){
			balance(nNode);
			return;
		}
	...
	//===============================
	private void balance(AVL_Node nNode){
		...
		else if (nNode.getBalanceFactor() < 0){
	...
	public int[] bfCount(AVL_Node cur){
		int[] arr = {0,0,0};
		return bfCountHelper(cur,arr);
	}
	public int[] bfCountHelper(AVL_Node cur, int[] arr){
		
		if(cur == null)
			return arr;
		else{
			if(cur.getBalanceFactor() == -1)
				arr[0]++;
			else if (cur.getBalanceFactor() == 0)
				arr[1]++;
			else
				arr[2]++;
			bfCountHelper(cur.getLeftChild(),arr);
			bfCountHelper(cur.getRightChild(),arr);
		}
		return arr;
	}
